...
pre_pax_counts <- window(ts_pax_counts, start = pre_pandemic_start, end = pre_pandemic_end)
pandemic_pax_counts = window(ts_pax_counts, start = pandemic_start, end = pandemic_end)
post_pax_counts = window(ts_pax_counts, start = pandemic_end, end = post_pandemic_end)

#Domestic Flight Revenue
ts_revenue <- ts(airline_dataset[, c("Dom_RPM")], start = c(2003, 1), frequency = 12)
pre_revenue <- window(ts_revenue, start = pre_pandemic_start, end = pre_pandemic_end)
pandemic_revenue <- window(ts_revenue, start = pandemic_start, end = pandemic_end)
post_revenue <- window(ts_revenue, start = pandemic_end, end = post_pandemic_end)

#Domestic Load Factors
ts_LF <- ts(airline_dataset[, c("Dom_LF")], start = c(2003, 1), frequency = 12)
pre_LF <- window(ts_LF, start = pre_pandemic_start, end = pre_pandemic_end)
pandemic_LF <- window(ts_LF, start = pandemic_start, end = pandemic_end)
post_LF <- window(ts_LF, start = pandemic_end, end = post_pandemic_end)
...
#### As shown in our project proposal, our data is not stationary, which means differencing is required
```{r}
ur.kpss(pre_pax_counts) %>% summary() #Non-stationary
ur.kpss(pandemic_pax_counts) %>% summary() #Non-stationary

ur.kpss(pre_revenue) %>% summary() #Non-stationary
ur.kpss(pandemic_revenue) %>% summary() #Non-stationary

ur.kpss(pre_LF) %>% summary() #Non-stationary
ur.kpss(pandemic_LF) %>% summary() #Non-stationary
```
#### Conducting seasonal differencing on the pre-pandemic data as it exhbits strong seasonality
```{r}
pre_paxDiff <- diff(log(pre_pax_counts),lag = 12)
pre_revDiff <- diff(log(pre_revenue),lag = 12)
pre_LFDiff <- diff(log(pre_LF),lag = 12)
```

#### Conducting normal differencing on pandemic data

```{r}
pandemic_paxDiff <- diff(pandemic_pax_counts, lag = 1)
pandemic_revenueDiff <- diff(pandemic_revenue, lag = 1)
pandemic_LFDiff <- diff(pandemic_LF, lag = 1)
```

#Conduct KPSS testing again to determine effectiveness of differencing
```{r}
ur.kpss(pre_paxDiff) %>% summary() #Stationary
ur.kpss(pandemic_paxDiff) %>% summary() #Stationary

ur.kpss(pre_revDiff) %>% summary() #Stationary
ur.kpss(pandemic_revenueDiff) %>% summary() #Stationary

ur.kpss(pre_LFDiff) %>% summary() #Non-Stationary
ur.kpss(pandemic_LFDiff) %>% summary() #Stationary
```
After conducting differencing, the only dataset that is still not stationary is the pre-pandemic load factor dataset. Because seasonal differencing was already conducted on the dataset, we will now conduct normal differencing.

# Differencing the pre-pandemic load factor dataset
```{r}
